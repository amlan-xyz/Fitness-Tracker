...
//actions
import {fetchGoals,deleteGoals} from '../../actions/goals.actions'
                ...
                loading===true?<Loader/>:<ul className="list">
                {
                    goals && goals.map(goal=>(
                        <li className="list-item" key={goal._id}>
                            <button onClick={()=>handleDelete(goal._id)}><AiOutlineDelete/></button>
                            <h3>
                               {goal.goal_name}
                            </h3>
                            <p>{goal.goal_description}</p>
                            <p>{
                                convertDate(goal.target_date)    
                            }</p>
                            <p>{goal.target_calories} calories</p>
                            <p className={goal.goal_status==="in-progress"?"highlight-red":"highlight-green"}>{goal.goal_status}</p>
                        </li>
                    ))
                }
            </ul>
